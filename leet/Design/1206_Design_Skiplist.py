...
class Skiplist:
    ...
    def verbose(self, head, level):
        vals = []
        node = head
        while node:
            vals.append(node.val)
            node = node.next
        print(f"Level {level} horizontal vals {vals}")
        node = head
        vals = []
        while node:
            vals.append(node.val)
            node = node.down
        print(f"Level {level} down vals {vals}")

    def add(self, num: int) -> None:
        [self.verbose(node, i) for i, node in enumerate(self.levels)]
        stack = []
        level = self.levels[-1]
        while level:
            node = level
