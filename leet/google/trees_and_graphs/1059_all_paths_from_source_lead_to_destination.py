        ...
        def dfs(node):
            if colors[node] == WHITE:
                ...
                for child in graph[node]:
                    # once cycle detected, return False
                    if not dfs(child):
                        return False
                colors[node] = BLACK
                # if vertex has no children and it is not a destination we return False
                return len(graph[node]) > 0 or node == destination
            else:
                return colors[node] == BLACK
                # since the problem states "only if all roads from source lead to destination" we start from source
                return dfs(source)

if __name__ == '__main__':
    s = Solution()
    s.leadsToDestination(50,
